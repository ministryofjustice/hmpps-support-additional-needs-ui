...
describe('history-delete/confirm/historyConfirmController', () => {
  ...
  const controller = new HistoryConfirmController(challengeService, auditService)
  ...
  describe('submitConfirmForm', () => {
    it('should delete history challenge, fire audit with mode: history, clear journey data, and redirect with success', async () => {
      // Given
      ...
      expect(res.redirectWithSuccess).toHaveBeenCalledWith(expectedNextRoute, 'History challenge deleted.')
