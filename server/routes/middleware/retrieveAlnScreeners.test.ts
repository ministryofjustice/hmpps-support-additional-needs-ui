...
describe('retrieveAlnScreeners', () => {
  ...
  const requestHandler = retrieveAlnScreeners(alnService)
  ...
  it('should retrieve ALN Screeners and store on res.locals', async () => {
    ...
    alnService.getAlnScreeners.mockResolvedValue(expectedAlnScreenerList)
    ...
    expect(alnService.getAlnScreeners).toHaveBeenCalledWith(username, prisonNumber)
  ...
  it('should store un-fulfilled promise on res.locals given service returns an error', async () => {
    ...
    alnService.getAlnScreeners.mockRejectedValue(error)
