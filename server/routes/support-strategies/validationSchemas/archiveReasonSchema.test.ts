...
describe('archiveReasonSchema', () => {
  ...
  beforeEach(() => {
    jest.resetAllMocks()
    req.originalUrl =
      '/support-strategies/A1234BC/187168d1-121c-42bf-b456-47711924ffa4/archive/473e9ee4-37d6-4afb-92a2-5729b10cc60f/reason'
  })

  it('happy path - validation passes', async () => {
    // Given
    const requestBody = { archiveReason: 'This support strategy was incorrectly added by mistake' }
  ...
  it.each([
    ...
    const expectedErrors: Array<Error> = [
      {
        href: '#archiveReason',
        text: 'Add reason for moving this support strategy to the History',
      },
    ]
  ...
  it('sad path - archiveReason field length validation fails', async () => {
    ...
    const expectedErrors: Array<Error> = [
      {
        href: '#archiveReason',
        text: 'Reason for moving this support strategy to the History must be 4000 characters or less',
      },
    ]
    const expectedInvalidForm = JSON.stringify(requestBody)
