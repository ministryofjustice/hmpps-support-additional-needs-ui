...
import type { StrengthListResponse, ReferenceData } from 'supportAdditionalNeedsApiClient'
import type { StrengthResponseDto } from 'dto'
import { toStrengthDto } from './strengthDtoMapper'

describe('toStrengthDto', () => {
  const prisonNumber = 'A1234BC'

  it('should map a single strength correctly', () => {
    const testRef = 'abcdef'
    const apiResponse: StrengthListResponse = {
      strengths: [
        {
          reference: testRef,
          id: 'CH001',
          ...
          categoryCode: 'CC001',
          detail: 'Test detail',
          strengthType: { code: 'TYPE001', categoryCode: 'EMOTIONS_FEELINGS' } as ReferenceData,
    ...
    const result = toStrengthDto(prisonNumber, apiResponse)

    expect(result).toHaveLength(1)
    expect(result[0]).toEqual<StrengthResponseDto>({
      prisonNumber,
      createdAtPrison: 'PR001',
  ...
  it('should map a multiple strengths correctly', () => {
    ...
    expect(result[1]).toEqual<StrengthResponseDto>({
      prisonNumber,
      createdAtPrison: 'PR001',
      strengthCategory: 'EMOTIONS_FEELINGS',
      createdAt: parseISO('2025-07-25T12:00:00.000Z'),
  ...
  it('should handle empty `strengths` array', () => {
    const apiResponse: StrengthListResponse = { strengths: [] }
    const result = toStrengthDto(prisonNumber, apiResponse)
