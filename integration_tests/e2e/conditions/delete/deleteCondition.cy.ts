...
context('Delete a Condition (Current tab)', () => {
  ...
  it("should delete a prisoner's condition via the journey, triggering validation on every screen", () => {
    // Given
    cy.signIn()
    ...
    Page.verifyOnPage(ConditionsPage) //
      .hasSuccessMessage('Condition deleted.')
  ...
  it('should not show the Delete link to a user without DELETE_CONDITIONS permission', () => {
    ...
    Page.verifyOnPage(ConditionsPage) //
      .doesNotHaveDeleteConditionButton()
  })

  it('should not be able to navigate directly to the delete condition page given user does not have the required role', () => {
    // Given - sign in with a role that doesn't have DELETE_CONDITIONS permission
    cy.task('stubSignIn', { roles: ['ROLE_SOME_OTHER_ROLE'] })
    cy.signIn()

    // When - visit the delete URL directly
    cy.visit(`/conditions/${prisonNumber}/${conditionReference}/delete/reason`, { failOnStatusCode: false })
