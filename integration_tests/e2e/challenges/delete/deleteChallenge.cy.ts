...
context('Delete a Challenge (Current tab)', () => {
  ...
  it("should delete a prisoner's challenge via the journey, triggering validation on every screen", () => {
    // Given
    cy.signIn()
  ...
  it('should not be able to navigate directly to the delete challenge page given user does not have the required role', () => {
    // Given - sign in with a role that doesn't have DELETE_CHALLENGES permission
    cy.task('stubSignIn', { roles: ['ROLE_SOME_OTHER_ROLE'] })
    cy.signIn()

    // When - visit the delete URL directly
    cy.visit(`/challenges/${prisonNumber}/${challengeReference}/delete/reason`, { failOnStatusCode: false })
  ...
  it('should redirect back to confirm with an error banner if the API returns an error on confirm', () => {
    ...
    cy.visit(`/challenges/${prisonNumber}/${challengeReference}/delete/reason`)
    Page.verifyOnPage(DeleteChallengeReasonPage) //
      .selectReason('ENTERED_IN_ERROR')
      .submitPageTo(DeleteChallengeReviewPage)
